...
export const ApplicationRepository = {
  ...
  createRoomStandards: async (payload: any) => {
    try {
      ...
      const fallbackProjectId = "1003";
      const project_id = payload.projectId || fallbackProjectId;
      console.log('Inserting room standards with project_id:', project_id, 'and payload:', payload);
      const [result] = await database.execute(
  ...
  createZoneRooms: async (payload: any) => {
    ...
    const project_standard_id = payload.projectStandardId ?? fallbackProjectStandardId;
    try {
      const [result] = await database.execute(
