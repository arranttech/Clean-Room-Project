...
export const ApplicationRepository = {
	...
	getScreens: async () => {
		try {
			const [rows] = await database.execute(`SELECT * FROM tScreens ORDER BY screen_id ASC`);
			return rows;
		} catch (err) {
			// Fast fail if table missing
			if ((err as any).code === 'ER_NO_SUCH_TABLE') {
				console.warn("Table tScreens missing. Ensure database initialization is correct.");
				return [];
			}
	...
	createScreen: async (payload: any) => {
		try {
			const [result] = await database.execute(
				`INSERT INTO tScreens (screen_name, screen_status) VALUES (?, ?)`,
	...
	updateScreen: async (id: number, payload: any) => {
		try {
			const [result]: any = await database.execute(
				`UPDATE tScreens SET screen_status = ? WHERE screen_id = ?`,
				[payload.status, id]
			);
			return result.affectedRows > 0;
		} catch (err) {
