...
export function boqresults(
  ...
  try {
    ...
    const allResults = Array.isArray(result) ? result : result ? [result] : [];

    const currentZoneSystem = zone?.zoneSystem ?? room?.zoneSystem ?? "";

    const currentZoneSystemType = room?.zoneSystemType ?? "";

    const { showCooling, showHeating, isVentilationSystem } = getSystemFlags(
      currentZoneSystem,
    ...
    const exhaustValues = allResults.map((r: any) => toNumber(r?.exhaustAir));
    ...
    const isSinglePipe =
      (pipeConfiguration || "").toUpperCase() === "SINGLE PIPE";

    const ChilledWaterGPM = Math.max(zoneRoomACValue, zoneCfmACLoadTR) * 4;

    console.log("Calculated Chilled Water GPM:", ChilledWaterGPM);

    const HotWaterGPM =
      Math.max(zoneRoomHeatLoadTR, zoneCfmHeatLoadTRValue) * 4;
    ...
    function calculatedAHUCfm(): number {
      ...
      const ventilationSupplyCfm = Math.ceil((roomCFM + freshAir) / 250) * 250;

      const ventilationExhaustCfm = Math.ceil(roomCFM / 250) * 250;
      ...
      if (isCoolingSupply) ahuCfm = coolingSupplyCfm;
        ...
        ahuCfm = isSupplySystem ? ventilationSupplyCfm : ventilationExhaustCfm;
    ...
    function calculateAHULength(
      ...
      const currentSystemLower = String(currentZoneSystem || "").toLowerCase();
      console.log("currentSystemLower:", currentSystemLower);
      const isPureVentilationSystem =
        currentSystemLower.includes("ventilation") &&
        !currentSystemLower.includes("heating") &&
        !currentSystemLower.includes("cooling");

      const ahufltrval = isPureVentilationSystem
        ? ahuln.VentilationFilterLength || ahuln.FilterLength || []
        : ahuln.FilterLength || [];
    ...
    function displayflowvelocity(): number | number[] {
      ...
      if (isHeatingandCooling && isSinglePipe) {
        ...
        if (isHeatingSupply) {
          ...
          return Number(
            ((coolingFlowVelocity * HotWaterGPM) / ChilledWaterGPM).toFixed(3),
          );
        }
...
const calculateSharedSupplyAHULength = (
  ...
  stages: number,
  coilRowsList: number[],
  useVentilationFilterLength = false,
): number => {
  const ahu = boqresult?.fields?.AHUSize;
  const ahuln = ahu?.AHULengthCfm;
  ...
  const filterLengths = useVentilationFilterLength
    ? ahuln.VentilationFilterLength || ahuln.FilterLength || []
    : ahuln.FilterLength || [];
...
export function getBOQRowsForZone(
  ...
  const system = String(
    zone?.zoneSystem ?? room?.zoneSystem ?? "",
  ).toLowerCase();
  ...
  const isCoolingVentilation =
    system.includes("cooling") && system.includes("ventilation");

  const isHeatingVentilation =
    system.includes("heating") && system.includes("ventilation");
