...
interface AccumulatorTotals {
    ...
    zoneHumidValue: number; zoneResultantHeatCfm: number; zoneRoomTermSupplyHeatValue: number;
    zoneCfmHeatLoadTRValue: number; zoneRoomHeatLoadTR: number; zoneResultHeatLoadTR: number;
}
...
export function cumulativeZoneService(
    zoneName: string,
    rooms: ZonePayload[]
): { exhaustTotals: CalculatedZoneResults; nonExhaustTotals: CalculatedZoneResults } {
    ...
    const createEmptyTotal = (): AccumulatorTotals => ({
        ...
        zoneHumidValue: 0, zoneResultantHeatCfm: 0, zoneRoomTermSupplyHeatValue: 0,
        zoneCfmHeatLoadTRValue: 0, zoneRoomHeatLoadTR: 0, zoneResultHeatLoadTR: 0,
    });

    const splitTotals = rooms.reduce(
        (acc, room) => {
            const isExhaust = ensureNumber(room.exhaustAir) !== 0;
            const target = isExhaust ? acc.exhaust : acc.nonExhaust;
            ...
            target.zoneRoomTermSupplyHeatValue += ensureNumber(room.roomTermSupplyHeatValue);
        ...
        { exhaust: createEmptyTotal(), nonExhaust: createEmptyTotal() }
    );

    const formatResults = (totals: AccumulatorTotals): CalculatedZoneResults => ({
        zoneName,
        zoneSystem,
        zoneClassification,
        zoneReqInsideTempC,
    ...
    return {
        exhaustTotals: formatResults(splitTotals.exhaust),
        nonExhaustTotals: formatResults(splitTotals.nonExhaust),
    };
}