...
export function cumulativeZoneService(zoneName: string, rooms: ZonePayload[]): CalculatedZoneResults {
    ...
    const totals = rooms.reduce((acc, room) => {
        return {
            ...
            zoneDehumidValue: acc.zoneDehumidValue + ensureNumber(room.dehumidValue),
            zoneRemovedWater: acc.zoneRemovedWater + ensureNumber(room.removedWater),
            zoneResultantCfm: acc.zoneResultantCfm + ensureNumber(room.resultantCfm),
            zoneRoomACValue: acc.zoneRoomACValue + ensureNumber(room.roomACValue),
            zoneRoomTermSupplyValue: acc.zoneRoomTermSupplyValue + ensureNumber(room.roomTermSupplyValue),
            zoneCfmACLoadTR: acc.zoneCfmACLoadTR + ensureNumber(room.cfmACLoadTR),
            zoneResultCoolLoadTR: acc.zoneResultCoolLoadTR + ensureNumber(room.resultCoolLoadTR),
            zoneAddWaterValue: acc.zoneAddWaterValue + ensureNumber(room.addWaterValue),
            zoneHumidValue: acc.zoneHumidValue + ensureNumber(room.humidValue),
            zoneResultantHeatCfm: acc.zoneResultantHeatCfm + ensureNumber(room.resultantheatCfm),
            zoneRoomTermSupplyHeatValue: acc.zoneRoomTermSupplyHeatValue + ensureNumber(room.roomTermSupplyHeatValue),
            zoneCfmHeatLoadTRValue: acc.zoneCfmHeatLoadTRValue + ensureNumber(room.cfmHeatLoadTRValue),
            zoneRoomHeatLoadTR: acc.zoneRoomHeatLoadTR + ensureNumber(room.roomHeatLoadTR),
            zoneResultHeatLoadTR: acc.zoneResultHeatLoadTR + ensureNumber(room.resultHeatLoadTR),
        };
    }, {
        zoneName: zoneName,
        zoneArea: 0, zoneVolume: 0, zoneRoomCfm: 0, zoneFreshAir: 0, zoneExhaustAir: 0,
        zoneDehumidValue: 0, zoneRemovedWater: 0, zoneResultantCfm: 0, zoneRoomACValue: 0,
        zoneRoomTermSupplyValue: 0, zoneCfmACLoadTR: 0, zoneResultCoolLoadTR: 0,
        zoneAddWaterValue: 0, zoneHumidValue: 0, zoneResultantHeatCfm: 0,
    ...
    return {
        ...
        zoneRoomTermSupplyHeatValue: Number(totals.zoneRoomTermSupplyHeatValue.toFixed(2)),
