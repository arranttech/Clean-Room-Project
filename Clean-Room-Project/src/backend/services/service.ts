...
export function airflowService(room: RoomPayload): AirflowResults {
	...
	// ================= SYSTEM FLAGS =================
	const { showCooling, showHeating, isTempValid } = getSystemFlags(
		room.zoneSystem || "",
		room
	);
	...
	const faPercent = isVentilationSystem
		? 100
	...
	function calculateResultCoolLoadTR() {
		...
		if (showCooling && isTempValid) {
			resultCoolLoadTR =
				Math.ceil(Math.max(Number(roomACValue), Number(cfmACLoadTR)) / 0.5) *
				0.5;
		} else {
	...
	function calculateResultantHeatCfm() {
		...
		if (showHeating && isTempValid) {
			...
			resultantheatCfm =
				Math.ceil(Math.max(roomCfm + freshAir, humidValNum) / 25) * 25;
		}
	...
	function calculateResultantHeatLoadTR() {
		...
		if (showHeating && isTempValid) {
			resultHeatLoadTR =
				Math.ceil(
					Math.max(Number(roomHeatLoad || 0), Number(cfmHeatLoadTRValue || 0)) /
						0.5
				) * 0.5;
		} else {
