...
function calcTotals(results: any[]) {
  return results.reduce(
    (acc, curr) => ({
      ...
      heatroomtermsup: acc.heatroomtermsup + (Number(curr.heatroomtermsup) || 0),
      roomHeatLoad: acc.roomHeatLoad + (Number(curr.roomHeatLoad) || 0),
      cfmHeatLoadTRValue: acc.cfmHeatLoadTRValue + (Number(curr.cfmHeatLoadTRValue) || 0),
      resultHeatLoadTR: acc.resultHeatLoadTR + (Number(curr.resultHeatLoadTR) || 0),
    }),
    {
      ...
      AddWaterVapour: 0, humidcfm: 0, resultantCfm: 0, heatroomtermsup: 0,
      cfmHeatLoadTRValue: 0, roomHeatLoad: 0, resultHeatLoadTR: 0,
    },
  );
}
...
export default function Results() {
  ...
  useEffect(() => {
    ...
    zids.forEach((zid) => {
      ...
      console.log(`Zone ${zid}: ${zr.length} rooms | System: ${zr[0]?.zoneSystem}`);
      zr.forEach((r, i) => console.log(`  ${i + 1}: ${r.roomName} ACPH:${r.acph}`));
    });
    console.log("=========================");
  }, [rooms]);
  ...
  const computeZone = (zoneRooms: RoomForm[]) => {
    ...
    const zoneSystemName = String(first.zoneSystem || "").toUpperCase().trim();
    ...
    const isTempValid = !isNaN(reqInsideTemp) && first.zoneReqInsideTempC !== "";
    ...
    const results = zoneRooms.map((room) => {
      ...
      if (V1.ISO8Cd.includes(Classifi) || V1.ISO7Cd.includes(Classifi)) { result = Value / V2.ISO8VV; }
      else if (V1.ISOCd6.includes(Classifi)) { result = Value / V2.ISO6VV; }
      else if (V1.ISOCd5.includes(Classifi)) { result = Value / V2.ISO5VV; }
      else if (V1.ISOCd4.includes(Classifi)) { result = Value / V2.ISO4VV; }
      else if (V1.ISOCd3.includes(Classifi)) { result = Value / V2.ISO3VV; }
      else if (V1.ISOCd2.includes(Classifi)) { result = Value / V2.ISO2VV; }
      else if (V1.ISOCd1.includes(Classifi)) { result = Value / V2.ISO1VV; }
      else { result = Value / V2.NCVV; }
      ...
      if (isTempValid) {
        ...
        resultCoolLoadTRValue = Math.ceil(Math.max(Number(roomACValue), Number(cfmACLoadTRValue)) / 0.5) * 0.5;
      } else {
        cfmACLoadTRValue = first.zoneReqInsideTempC || "Invalid";
      ...
      if (showHeating && isTempValid) {
        ...
        addWaterValue = Math.abs(Number((freshAir * frAirCal * (delWater / c2.value3)).toFixed(3)));
  ...
  return (
    <div className={s.wrap}>
      <div className={s.card}>
        ...
        {zoneData.map(({ zoneId, results, showCooling, showHeating, isHeatingandCoolingSystem, zoneSystem, zoneStandard, zoneClassification }) => {
          ...
          return (
            <div key={zoneId} style={{ marginBottom: "48px" }}>
              ...
              >
                Zone {zoneId} — {zoneSystem} ({zoneStandard} / {zoneClassification})
              </h2>
              ...
              {isHeatingandCoolingSystem ? (
                <>
                  ...
                  <div className={s.tableOuter} style={{ marginBottom: "40px" }}>
                      ...
                      <table className={s.table}>
                        ...
                        <tbody>
                          ...
                          <tr className={s.tr} style={{ fontWeight: "bold" }}>
                            ...
                            <td className={s.td}>{totals.freshAir.toFixed(2)}</td>
                            <td className={s.td}>{totals.exhaustAir.toFixed(2)}</td>
                            <td className={s.td}>{totals.AddWaterVapour.toFixed(3)}</td>
                    ...
                    <table className={s.table}>
                      ...
                      <tbody>
                        {results.length > 0 ? (
                          ...
                          <tr>
                            <td className={s.emptyRow} colSpan={20}>No rooms added yet.</td>
                          </tr>
                        )}
                        {results.length > 0 && (
                          <tr className={s.tr} style={{ fontWeight: "bold" }}>
