...
export default function Room() {
	...
	// ─── Standards slice ───
	const rawZoneId = useAppSelector(
		(state: any) => state.standards.zoneId ?? "1"
	);
	...
	const saveCurrentRoom = async () => {
		...
		try {
			...
			await saveZoneRooms(roomToSave); // ✅ PASS DATA + WAIT
			setSelectedAcph(standardsAcph ?? "");
		} catch (error) {
	...
	const goToResultsPage = async () => {
		...
		zoneIds.forEach((zid) => {
			const zRooms = savedRooms.filter(
				(r: any) => String(r.zoneId || "1") === String(zid)
			);
			console.log(
		...
		try {
			// 1️⃣ Fetch and calculate airflow for all rooms
	...
	const addAnotherZone = () => {
		if (!zoneRooms.length) {
			alert(
				"Please add at least one room to the current zone before adding another."
			);
			return;
		}
								...
								savedRooms.map((r: any, i: number) => (
									<div key={r.id || i} className={s.roomCard}>
										...
										<div className={s.roomCardLine}>
											Zone: {r.zoneId ?? "1"} | System: {r.zoneSystem || "-"}
										</div>
										<div className={s.roomCardLine}>
