...
export default function Customers({ onCountChange }: CustomersProps) {
	...
	const handleEdit = async (customerId: number) => {
		try {
			...
			// backend returns { customer: {...} } — extract the inner object
			const customer = data.customer ?? data;
			console.log("Editing customer:", customer); // debug — remove after confirmed working
	...
	// Show AddCustomer form — passes editCustomer (non-null = edit mode, null = add mode)
	if (showAdd) {
		return (
			<AddCustomer
				customer={editCustomer}
				onCancel={() => { setShowAdd(false); setEditCustomer(null); }}
				onSaved={async () => { setShowAdd(false); setEditCustomer(null); await loadCustomers(); }}
			/>
		);
	}
	...
	return (
		<div>
			<div className={s.panelHeader}>
				...
				<button type="button" onClick={() => { setEditCustomer(null); setShowAdd(true); }} className={s.addBtn}>
				...
				{(["ALL", "A", "I"] as const).map((f) => (
					<button
						...
						className={`px-4 py-1.5 rounded-full text-xs font-semibold border transition-all ${
							statusFilter === f
								? f === "I" ? "bg-red-500 text-white border-red-500"
								: f === "A" ? "bg-green-500 text-white border-green-500"
			...
			<div className={s.searchWrap}>
				...
				<input type="text" className={s.searchInput} placeholder="Search customers..." value={search} onChange={(e) => setSearch(e.target.value)} />
			</div>
							...
							filtered.map((customer) => (
								<tr key={customer.customer_id} className={s.tr}>
									...
									<td className={s.tdActions}>
										...
										<button
											className={customer.status === "I" ? "text-slate-300 cursor-not-allowed p-1.5 rounded-lg" : s.deleteBtn}
											title={customer.status === "I" ? "Already inactive" : "Set Inactive"}
											disabled={customer.status === "I"}
											onClick={() => customer.status !== "I" && setDeleteTarget(customer)}
										>
											<FiTrash2 size={15} />
										</button>
									</td>
			...
			{deleteTarget && (
				<div className={s.deleteOverlay}>
					<div className={s.deleteBackdrop} onClick={() => setDeleteTarget(null)} />
					<div className={s.deleteCard}>
						...
						<p className={s.deleteMessage}>
							...
							<span className={s.deleteCustomerName}>{deleteTarget.customer_name}</span>?
							This action cannot be undone.
						</p>
						<div className={s.deleteBtnRow}>
							<button type="button" onClick={() => setDeleteTarget(null)} className={s.deleteCancelBtn}>
								<FiX className="text-base" /> No, Cancel
							</button>
							<button type="button" onClick={handleDeleteConfirm} className={s.deleteConfirmBtn}>
