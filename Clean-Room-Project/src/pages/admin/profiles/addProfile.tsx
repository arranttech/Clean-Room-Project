...
export default function AddProfile({ initialData, onCancel, onSaved }: AddProfileProps) {
    const [profileName, setProfileName] = useState(initialData?.name || "");
    const [profileDescription, setProfileDescription] = useState(initialData?.description || "");
    ...
    const SCREENS = [
        "Dashboard", "Project Creation", "Room Details", "Classification",
        "Company Profile", "Admin Management", "User Management", "Reports",
        "Project Information", "Customer Details", "Results Page"
    ];
    ...
    const handlePermissionChange = (screen: string, permission: "Full Access" | "Read Only" | "None") => {
        setPermissions(prev => ({ ...prev, [screen]: permission }));
    };

    const validateProfileName = (v: string) => v.trim().length > 0 ? "" : "Profile Name is required";
    const validateProfileDescription = (v: string) => v.trim().length > 0 ? "" : "Profile Description is required";

    const nameError = touched ? validateProfileName(profileName) : "";
    const descriptionError = touched ? validateProfileDescription(profileDescription) : "";

    const isPermissionsValid = !initialData || SCREENS.every(screen => permissions[screen] !== "");
    const isFormValid = profileName.trim().length > 0 && profileDescription.trim().length > 0 && isPermissionsValid;
    ...
    return (
        <div>
            <h1 className={s.formTitle}>{initialData ? "Edit Profile" : "Create New Profile"}</h1>
            <div className={s.formCard}>
                ...
                <div className={s.formRow}>
                    <div className={s.formGroup}>
                        ...
                        />
                        {descriptionError && <p className={s.formError}>{descriptionError}</p>}
                    </div>
                </div>
                ...
                {initialData && (
                    <>
                            ...
                            <table className={s.table}>
                                <thead className={s.thead}>
                                    <tr>
                                        ...
                                        <th className="px-5 py-4 text-center text-xs font-bold text-slate-700 tracking-wider">Full Access</th>
                                        <th className="px-5 py-4 text-center text-xs font-bold text-slate-700 tracking-wider">Read Only</th>
                                        <th className="px-5 py-4 text-center text-xs font-bold text-slate-700 tracking-wider">None</th>
                                    </tr>
                                </thead>
                        ...
                        {!isPermissionsValid && touched && (
                            <p className={`${s.formError} mt-2 text-right`}>Please select permission for all screens.</p>
                        )}
                    </>
                )}
                ...
                <div className={s.formFooter}>
                    ...
                    <button
                        ...
                        disabled={saving}
                    >
                        <FaFloppyDisk /> {saving ? "Saving..." : initialData ? "Save Changes" : "Create Profile"}
                    </button>
                </div>
            </div>
                ...
                <div className={s.popupOverlay}>
                    ...
                    <div className={s.popupCard}>
                        ...
                        <p className={s.popupMessage}>
                            Profile has been {initialData ? "updated" : "created"} successfully.
                        </p>
