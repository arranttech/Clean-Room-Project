...
export default function AssignProfile({ onCancel, onSaved }: AssignProfileProps) {
            ...
            <div className={s.assignCard}>
                <div className={s.assignGrid}>
                    ...
                    <div className={s.assignColWrap}>
                        <h2 className={s.assignColTitle}>Select User <span className={s.formRequired}>*</span></h2>
                            ...
                            <div className={s.assignList}>
                                {MOCK_USERS.map((user) => (
                                    <label key={user.id} className={s.assignListItem}>
                                        ...
                                        />
                                        <span className={selectedUserId === user.id ? s.assignLabelActive : s.assignLabel}>
                                            {user.name}
                                        </span>
                                    </label>
                                ))}
                    ...
                    <div className={s.assignColWrap}>
                        <h2 className={s.assignColTitle}>Select Profile <span className={s.formRequired}>*</span></h2>
                        <div className={s.assignCol}>
                            ...
                            <div className={s.assignList}>
                                {MOCK_PROFILES.map((profile) => (
                                    <label key={profile.id} className={s.assignListItem}>
                                        ...
                                        />
                                        <span className={selectedProfileId === profile.id ? s.assignLabelActive : s.assignLabel}>
                                            {profile.name}
                                        </span>
                                    </label>
                                ))}
                ...
                <div className={s.assignFooterWrap}>
                    <button type="button" className={s.formCancelBtn} onClick={onCancel} disabled={saving}>
                        Cancel
                    </button>
                    <button
                        type="button"
                        onClick={handleAssign}
                        className={`${s.formSubmitBtn} ${(!selectedUserId || !selectedProfileId) ? 'opacity-50 cursor-not-allowed' : ''}`}
            ...
            {showPopup && (
                <div className={s.popupOverlay}>
                    ...
                    <div className={s.popupCard}>
                        ...
                        <p className={s.popupMessage}>Profile has been assigned successfully.</p>
