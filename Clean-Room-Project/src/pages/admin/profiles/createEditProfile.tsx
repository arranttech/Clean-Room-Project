        ...
        return (
            <AddProfile
                ...
                onSaved={(newProfile) => {
                    ...
                    if (editData) {
                        setProfiles((prev) =>
                            prev.map((p) => (p.id === editData.id ? { ...newProfile, id: editData.id } as ProfileToken : p))
                        );
                        setEditData(null);
                    } else {
                        setProfiles((prev) => [{ ...newProfile, id: Math.random().toString() } as ProfileToken, ...prev]);
                    }
                }}
            />
        );
        ...
        <div>
            ...
            <div className={s.panelHeader}>
                <div className={s.panelTitleWrap}>
                    ...
                    <p className={s.panelSubtitle}>Create new profiles or edit existing ones</p>
                </div>
                        ...
                        {filteredData.length > 0 ? (
                            filteredData.map((row) => (
                                <tr key={row.id} className={s.tr}>
                                    ...
                                    <td className={s.td}>
                                        <span className={row.status === "Active" ? s.badgeActive : s.badgeInProgress}>
                                            {row.status}
                                        </span>
                                    </td>
                                    <td className={s.td}>{row.created}</td>
                                    <td className={s.tdActions}>
                                        <button className={s.editBtn} title="Edit profile" onClick={() => setEditData(row)}>
                                            <FiEdit2 size={16} />
                                        </button>
                                        <button className={s.deleteBtn} title="Delete profile" onClick={() => handleDelete(row.id)}>
                                            <FiTrash2 size={16} />
                                        </button>
                                    </td>
                                </tr>
