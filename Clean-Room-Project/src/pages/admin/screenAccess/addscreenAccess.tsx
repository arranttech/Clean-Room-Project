...
export default function AddscreenAccess({
	initialData,
	onCancel,
	onSaved,
}: AddscreenAccessProps) {
	...
	const validateRoleAccess = (v: string) =>
		v.length > 0 ? "" : "Role Access is required";
	...
	return (
		...
		<div>
			<h1 className={s.formTitle}>
				{initialData ? "Edit" : "Add New"} Screen Access
			</h1>
			.{" "}
			{/* ---edit screen access if initialData is provided, otherwise add new screen access--- */}
											...
											<td className={s.permissionTd}>
												<label className={s.permissionRadioWrapper}>
													<input
														...
														onChange={() =>
															handlePermissionChange(screen, "No Access")
														}
													/>
												</label>
											</td>
				...
				<div className={s.popupOverlay}>
					...
					<div className={s.popupCard}>
						...
						<h2 className={s.popupTitle}>
							{initialData ? "Screen Updated!" : "Screen Saved!"}
						</h2>
						<p className={s.popupMessage}>
							{initialData
								? "Screen access has been successfully updated."
								: "Screen has been successfully saved."}
						</p>
