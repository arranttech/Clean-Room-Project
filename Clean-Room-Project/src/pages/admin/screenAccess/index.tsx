	...
	if (showAdd || editData) {
		return (
			<AddscreenAccess
				...
				onCancel={() => {
					//if user cancels the add/edit operation, close the form and reset editData to null
					setShowAdd(false);
					setEditData(null);
				}}
				onSaved={(newScreen) => {
					//when user saves the new or edited screen access, update the screens state accordingly. If editData is not null, it means we are editing an existing screen, so we map through the screens and replace the old data with the new data. If editData is null, it means we are adding a new screen, so we add it to the beginning of the screens array.
					setShowAdd(false);
					if (editData) {
						setScreens((prev) =>
							prev.map((s) =>
								s.id === editData.id ? { ...newScreen, id: editData.id } : s
							)
						);
						setEditData(null);
					} else {
		...
		<div className={s.tooltipWrap}>
			...
			{constants.Tooltip.screenPermissionsTooltip.map((item, idx) => (
				<div key={idx} className={s.tooltipItem}>
					<span
						className={s[`tooltipDot${item.color}` as keyof typeof s]}
					></span>
				...
				<table className={s.table}>
					<thead className={s.thead}>
						<tr>
							...
							<th className={s.th}>
								Screen Permissions{" "}
								<Tooltip id="screenPermissions" content={tooltipContent} />
							</th>
									...
									<td className={s.td}>
										<div className="flex flex-wrap gap-2">
											{row.permissions
												? Object.entries(row.permissions).map(
														([screenName, level]) => {
															...
															return (
																<span
																	key={screenName}
																	className={`px-2.5 py-1 text-[10px] rounded-md ${badgeClass}`}
																>
																	{screenName}
																</span>
															);
												...
												: row.name &&
												  row.name.split(", ").map((screenName, idx) => (
														<span
															key={idx}
															className="px-2.5 py-1 text-[11px] font-bold rounded-md bg-slate-100 text-slate-700"
														>
															{screenName}
														</span>
												  ))}
