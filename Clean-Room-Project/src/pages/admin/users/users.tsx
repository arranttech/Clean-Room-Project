...
import { getUsers } from "../../../backend/controller/controller";
...
export default function Users({ onCountChange }: UsersProps) {
	...
	useEffect(() => {
		const fetchUsers = async () => {
			try {
				const data = await getUsers(); // number is allowed
						...
						{filtered.length === 0 ? (
							...
							filtered.map((user) => (
								<tr key={user.user_login_id} className={s.tr}>
									...
									<td className={s.td}>
										{user.update_date
											? new Date(user.update_date).toISOString().split("T")[0]
											: "N/A"}
									</td>
									...
									<td className={s.tdActions}>
										...
										<button
											type="button"
											onClick={() => handleDelete(user.user_login_id)}
