...
function CustomerInfoPage() {
  ...
  const loggedInUserId = useAppSelector(
    (state: any) => state.user?.user_login_id ?? null
  );
  ...
  useEffect(() => {
    ...
    // Otherwise fetch from DB using user_login_id from Redux
  ...
  const navigateAfterSave = (name: string) => {
  ...
  const saveCustomerInfo = async () => {
    if (isSaved && savedCustomerId) {
      ...
      toast.success("Details saved successfully!", {
        onClose: () => navigateAfterSave(customerName),
        autoClose: 1500,
      });
      return;
    }
