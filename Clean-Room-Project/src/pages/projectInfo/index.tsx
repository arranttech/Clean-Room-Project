...
function ProjectInfoPage() {
  ...
  const fixedClimateValues = [
    {
      minTemp: "-38.3",
      maxTemp: "21.5",
      relativeHumidityMin: "45",
      relativeHumidityMax: "100",
    },

    {
      minTemp: "-9.1",
      maxTemp: "23.3",
      relativeHumidityMin: "18",
      relativeHumidityMax: "100",
    },
  ...
  const getRandomClimateValues = () => {
    const randomIndex = Math.floor(Math.random() * fixedClimateValues.length);

    return fixedClimateValues[randomIndex];
  };
  ...
  const handleSelectLocation = async (place: any) => {
    ...
    if (!useWeatherApi) {
      dispatch(updateMultipleFields(getRandomClimateValues()));
      return;
    }
