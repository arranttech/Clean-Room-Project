const resetPasswordStyles = {
  ...
  verifyingText: "text-sm text-gray-500 text-center py-4",
  errorBox:
    "flex items-start gap-3 bg-red-50 border border-red-100 rounded-lg p-4 mb-4",
  errorBoxTitle: "text-sm font-semibold text-red-600",
  errorBoxMsg: "text-xs text-red-500 mt-0.5",
  requestNewLink:
    "text-xs text-teal-600 hover:underline mt-2 block font-medium",
  successWrap: "flex flex-col items-center gap-3 py-4",
  successIcon:
    "w-14 h-14 bg-green-50 rounded-full flex items-center justify-center mx-auto",
  ...
  eyeBtn:
    "absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-teal-600 transition",
