...
export default function Standard() {
	...
	// GET — only fetch if no standard set yet
	useEffect(() => {
		if (!projectId) return;
	...
	const classList = useMemo(() => {
		...
		return (selectedStandard.classifications || []).filter((c) => {
			const name = c.name || ""; // fallback to empty string
			const isNCClass =
	...
	const createProjectZones = async () => {
		...
		const payload = { project_id: freshProjectId };
		try {
	...
	const handleNext = async (e?: React.MouseEvent) => {
		...
		// 	dispatch(
		// 		updateStandardsField({ field: "zoneId", value: zoneData.zoneId })
		// 	);
		// 	console.log("zoneIdFromRedux", zoneIdFromRedux);
		// 	const standardData = await saveroomStandards();
		// 	if (standardData?.roomStandardsId && zoneIdFromRedux) {
		...
		// }
		try {
			let currentZoneId = zoneIdFromRedux;
			let currentProjectStandardId = projectStandardIdFromRedux;

			// Create zone only if it doesn't exist
			if (!currentZoneId) {
				const zoneData = await createProjectZones();
				...
				dispatch(
					updateStandardsField({ field: "zoneId", value: currentZoneId })
				);
			}

			// Create room standard only if it doesn't exist
			if (!currentProjectStandardId) {
				const standardData = await saveroomStandards();
				currentProjectStandardId = standardData?.roomStandardsId;

				if (!currentProjectStandardId) {
					alert("Failed to create project standard.");
					return;
				}
				...
				dispatch(
					updateStandardsField({
						field: "projectStandardId",
						value: currentProjectStandardId,
					})
				);
			}
			...
			navigate("/room", {
				state: {
					...
					projectStandardId: currentProjectStandardId,
				},
			});
		} catch (error) {
