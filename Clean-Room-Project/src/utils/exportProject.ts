...
function buildZoneSheet(
  ...
  const zoneInfoById = new Map<string, { zone_id: string; zone_name: string }>();
  for (const z of zones ?? [])
    zoneInfoById.set(String(z.zone_id), { zone_id: String(z.zone_id), zone_name: z.zone_name ?? `Zone ${z.zone_id}` });

  const zoneMap = new Map<string, { name: string; zoneId: string; rooms: any[] }>();
  for (const room of rooms) {
    const zid   = String(room.zone_id ?? "");
    const zname = room.zone_name && String(room.zone_name).trim() !== ""
      ? String(room.zone_name).trim()
      : zoneInfoById.get(zid)?.zone_name ?? `Zone ${zid}`;
    if (!zoneMap.has(zid))
      zoneMap.set(zid, { name: zname, zoneId: zid, rooms: [] });
    zoneMap.get(zid)!.rooms.push(room);
  }

  // Results keyed by room name
  ...
  for (const s of standards ?? []) stdById.set(String(s.project_standard_id), s);
  const firstStd = standards?.[0] ?? {};

  // tProjectZones rows keyed by zone_id 
  ...
  for (const [zoneId, { name: zoneName, rooms: zoneRooms }] of zoneMap.entries()) {
    ...
    const totalsRow: CO[] = COLS.map((col, i) => {
      ...
      return rdD(dbNum(zRow[col.zoneCol]), bl);     
    });
